...
# 	vf=ens1f$((i+2))
	...
	ovs-ofctl add-flow $br "table=0,in_port=$rep,ip,ct_state=-trk             actions=ct(table=10)"
        ovs-ofctl add-flow $br "table=10,in_port=$rep,ip,ct_state=+trk+new        actions=goto_table=20"
	ovs-ofctl add-flow $br "table=10,in_port=$rep,ip,ct_state=+trk+est-rpl,   action=ct_clear,goto_table=20"

	ovs-ofctl add-flow $br "table=20,in_port=$rep,ip,   actions=ct_clear,resubmit(,30)"

	ovs-ofctl add-flow $br "table=30,in_port=$rep,priority=10,ip,ct_state=-trk,action=load:$reg6->NXM_NX_REG6[],ct(nat,table=40)"
	ovs-ofctl add-flow $br "table=40,in_port=$rep,ip,reg6=$reg6,ct_state=+trk+new,action=ct(commit,nat(src=8.9.10.1:5000-50000),exec(move:NXM_NX_REG6[]->NXM_NX_CT_MARK[])),mod_dl_src:${MAC_ROUTE},mod_dl_dst:${REMOTE_PF_MAC},$pf"
	ovs-ofctl add-flow $br "table=40,in_port=$rep,ct_state=+trk+est-rpl,ip,action=mod_dl_src:${MAC_ROUTE},mod_dl_dst:${REMOTE_PF_MAC},$pf"
