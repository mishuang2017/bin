...
ip netns exec n11 ip route add 100.64.0.0/24 via 192.168.0.1 dev enp4s0f3
...
MAC_EX=`cat /sys/class/net/br-ex/address`
MAC_EX2=$(echo $MAC_EX | sed 's/://g' | sed 's/^/0x/')
echo $MAC_EX2
...
ovs-ofctl add-flow $br "table=0, in_port=$PF, dl_type=0x0806, nw_dst=100.64.0.10, actions=load:0x2->NXM_OF_ARP_OP[], move:NXM_OF_ETH_SRC[]->NXM_OF_ETH_DST[], mod_dl_src:${MAC_ROUTE}, move:NXM_NX_ARP_SHA[]->NXM_NX_ARP_THA[], move:NXM_OF_ARP_SPA[]->NXM_OF_ARP_TPA[], load:0x248a07ad7799->NXM_NX_ARP_SHA[], load:0x6440000a->NXM_OF_ARP_SPA[], in_port"
...
ovs-ofctl add-flow $br "table=70,priority=50,ip actions=move:NXM_OF_IP_SRC[]->NXM_NX_REG5[],move:NXM_NX_REG6[]->NXM_OF_IP_SRC[],load:0x1->NXM_OF_IP_SRC[31],ct(commit,table=71,zone=65534,nat(src=100.64.0.10),exec(move:NXM_NX_REG6[]->NXM_NX_CT_MARK[],move:NXM_NX_REG5[]->NXM_NX_CT_LABEL[0..31]))"
